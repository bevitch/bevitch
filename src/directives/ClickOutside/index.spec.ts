...
const createDirective = () => merge({}, ClickOutside);
...
describe('v-click-outside -> directive', () => {
  it('it has bind, update and unbind methods available', () => {
    expect(typeof ClickOutside.bind).toBe('function');
    expect(typeof ClickOutside.update).toBe('function');
    expect(typeof ClickOutside.unbind).toBe('function');
  });
  ...
  describe('bind', () => {
    ...
    it('adds an event listener to the element and stores the handlers on the element', () => {
      ...
      el[HANDLERS_PROPERTY].forEach((eventHandler: any) =>
